 ...
 */
Ext.define('Form.view.reminder.AbstractCalendar', {
    ...
    getWeekCount: function() {
        var days = Form.view.reminder.util.Date.diffDays(this.viewStart, this.viewEnd);
            ...
            for (d = 0; d < this.dayCount; d++) {
                if (evtsInView.getCount() > 0) {
                    var evts = evtsInView.filterBy(function(rec) {
                        var startDt = Ext.Date.clearTime(rec.data[Form.model.reminder.EventMappings.getFields()[0].StartDate.name], true),
                            startsOnDate = dt.getTime() == startDt.getTime(),
                            spansFromPrevView = (w == 0 && d == 0 && (dt > rec.data[Form.model.reminder.EventMappings.getFields()[0].StartDate.name]));
                ...
                dt = Form.view.reminder.util.Date.add(dt, {days: 1});
            }
        }
    ...
    prepareEventGrid: function(evts, w, d) {
        ...
        evts.each(function(evt) {
            var M = Form.model.reminder.EventMappings.getFields()[0],
            days = Form.view.reminder.util.Date.diffDays(
            Form.view.reminder.util.Date.max(me.viewStart, evt.data[M.StartDate.name]),
            Form.view.reminder.util.Date.min(me.viewEnd, evt.data[M.EndDate.name])) + 1;

            if (days > 1 || Form.view.reminder.util.Date.diffDays(evt.data[M.StartDate.name], evt.data[M.EndDate.name]) > 1) {
    ...
    onEventDrop: function(rec, dt) {
        if (Form.view.reminder.util.Date.compare(rec.data[Form.model.reminder.EventMappings.getFields()[0].StartDate.name], dt) === 0) {
            // no changes
            return;
        }
        var diff = dt.getTime() - rec.data[Form.model.reminder.EventMappings.getFields()[0].StartDate.name].getTime();
        rec.set(Form.model.reminder.EventMappings.getFields()[0].StartDate.name, dt);
        rec.set(Form.model.reminder.EventMappings.getFields()[0].EndDate.name, Form.view.reminder.util.Date.add(rec.data[Form.model.reminder.EventMappings.getFields()[0].EndDate.name], {millis: diff}));
    ...
    onCalendarEndDrag: function(start, end, onComplete) {
        if (start && end) {
            ...
            var o = {};
            o[Form.model.reminder.EventMappings.getFields()[0].StartDate.name] = start;
            o[Form.model.reminder.EventMappings.getFields()[0].EndDate.name] = end;
    ...
    onUpdate: function(ds, rec, operation) {
        ...
        if (operation == Ext.data.Record.COMMIT) {
            ...
            if (this.enableFx && this.enableUpdateFx) {
                this.doUpdateFx(this.getEventEls(rec.data[Form.model.reminder.EventMappings.getFields()[0].EventId.name]), {
                    scope: this
                });
            }
        }
    ...
    onAdd: function(ds, records, index) {
        ...
        if (this.enableFx && this.enableAddFx) {
            this.doAddFx(this.getEventEls(rec.data[Form.model.reminder.EventMappings.getFields()[0].EventId.name]), {
                scope: this
            });
        }
    },
    ...
    onRemove: function(ds, rec) {
        ...
        if (this.enableFx && this.enableRemoveFx) {
            this.doRemoveFx(this.getEventEls(rec.data[Form.model.reminder.EventMappings.getFields()[0].EventId.name]), {
                remove: true,
                scope: this,
            ...
            this.getEventEls(rec.data[Form.model.reminder.EventMappings.getFields()[0].EventId.name]).remove();
            this.refresh();
        }
    },
    ...
    isEventVisible: function(evt) {
    	try {
    	  	var M = Form.model.reminder.EventMappings.getFields()[0],
	            ...
	            evEnd = Form.view.reminder.util.Date.add(data[M.EndDate.name], {seconds: -1}).getTime();
    ...
    isOverlapping: function(evt1, evt2) {
        ...
        M = Form.model.reminder.EventMappings,
        start1 = ev1[M.getFields()[0].StartDate.name].getTime(),
        end1 = Form.view.reminder.util.Date.add(ev1[M.getFields()[0].EndDate.name], {seconds: -1}).getTime(),
        start2 = ev2[M.StartDate.name].getTime(),
        end2 = Form.view.reminder.util.Date.add(ev2[M.EndDate.name], {seconds: -1}).getTime();
    ...
    setViewBounds: function(startDate) {
        var start = startDate || this.startDate,
            ...
            Dt = Form.view.reminder.util.Date;
    ...
    sortEventRecordsForDay: function(evts) {
        ...
        evts.sortBy(Ext.bind(function(evtA, evtB) {
            var a = evtA.data,
            b = evtB.data,
            M = Form.model.reminder.EventMappings.getFields()[0];
            ...
            if (this.spansHavePriority) {
                ...
                var diff = Form.view.reminder.util.Date.diffDays;
                if (diff(a[M.StartDate.name], a[M.EndDate.name]) > 0) {
                    if (diff(b[M.StartDate], b[M.EndDate.name]) > 0) {
    ...
    moveNext: function(noRefresh) {
        return this.moveTo(Form.view.reminder.util.Date.add(this.viewEnd, {days: 1}));
    },
    ...
    movePrev: function(noRefresh) {
        var days = Form.view.reminder.util.Date.diffDays(this.viewStart, this.viewEnd) + 1;
    ...
    moveMonths: function(value, noRefresh) {
        return this.moveTo(Form.view.reminder.util.Date.add(this.startDate, {months: value}), noRefresh);
    },
    ...
    moveWeeks: function(value, noRefresh) {
        return this.moveTo(Form.view.reminder.util.Date.add(this.startDate, {days: value * 7}), noRefresh);
    },
    ...
    moveDays: function(value, noRefresh) {
        return this.moveTo(Form.view.reminder.util.Date.add(this.startDate, {days: value}), noRefresh);
    },
    ...
    getEventRecord: function(id) {
        var idx = this.store.find(Form.model.reminder.EventMappings.getFields()[0].EventId.name, id);
