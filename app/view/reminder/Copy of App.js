 ...
 */
Ext.define('Form.view.reminder.App', {
    alias: 'widget.reminderapp',
    requires: [
        'Ext.Viewport',
    ...
    constructor : function() {
        ...
        this.calendarStore = Ext.create('Form.store.reminder.MemoryCalendarStore', {
            data: Form.model.reminder.Calendars.getData()
        });

        // A sample event store that loads static JSON from a local file. Obviously a real
        // implementation would likely be loading remote data via an HttpProxy, but the
        // underlying store functionality is the same.
        this.eventStore = Ext.create('Form.store.reminder.MemoryEventStore', {
            data: Form.model.reminder.Events.getData()
        });
        ...
        Ext.create('Ext.Viewport', {
            ...
            items: [{
                id: 'app-center',
                title: 'eReminders', // will be updated to the current view's date range
                region: 'center',
                layout: 'border',
                listeners: {
                ...
                items: [{
                    ...
                    listeners: {
                        ...
                        'eventmove': {
                            fn: function(vw, rec){
                                var mappings = Form.model.reminder.EventMappings.getFields()[0],
    ...
    showEditWindow : function(rec, animateTarget){
        if(!this.editWin){
            this.editWin = Ext.create('Form.view.reminder.form.EventWindow', {
                calendarStore: this.calendarStore,
                listeners: {
                    'eventadd': {
                        fn: function(win, rec){
