 ...
 */
Ext.define('Form.view.reminder.DayBody', {
    extend: 'Form.view.reminder.AbstractCalendar',
    ...
    initDD: function() {
        ...
        this.dragZone = new Form.view.reminder.dd.DayDragZone(this.el, Ext.apply({
            view: this,
            containerScroll: true
        },
        cfg));

        this.dropZone = new Form.view.reminder.dd.DayDropZone(this.el, Ext.apply({
            view: this
        },
        cfg));
    },
    ...
    afterRender: function() {
        if (!this.tpl) {
            this.tpl = new Form.view.reminder.template.DayBody({
                id: this.id,
    ...
    onEventResize: function(rec, data) {
        var D = Form.view.reminder.util.Date,
        start = Form.model.reminder.EventMappings.getFields()[0].StartDate.name,
        end = Form.model.reminder.EventMappings.getFields()[0].EndDate.name;
    ...
    getTemplateEventData: function(evt) {
        var selector = this.getEventSelectorCls(evt[Form.model.reminder.EventMappings.getFields()[0].EventId.name]),
            data = {},
            M = Form.view.reminder.EventMappings.getFields()[0];
    ...
    getTemplateEventBox: function(evt) {
        var heightFactor = 0.7,
            start = evt[Form.model.reminder.EventMappings.getFields()[0].StartDate.name],
            end = evt[Form.model.reminder.EventMappings.getFields()[0].EndDate.name],
        ...
        for (; day < this.dayCount; day++) {
            ...
            for (; ev < ct; ev++) {
                ...
                evts.push({
                    ...
                    date: Form.view.reminder.util.Date.add(this.viewStart, {days: day})
                });
            }
        }
    ...
    getDayAt: function(x, y) {
        var xoffset = this.el.down('.ext-cal-day-times').getWidth(),
            ...
            mins = rowIndex * 30,
            dt = Form.view.reminder.util.Date.add(this.viewStart, {days: dayIndex, minutes: mins}),
            el = this.getDayEl(dt),
            timeX = x;
    ...
    onClick: function(e, t) {
        if (this.dragPending || Form.view.reminder.DayBody.superclass.onClick.apply(this, arguments)) {
