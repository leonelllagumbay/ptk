 ...
 */
Ext.define('Form.view.reminder.MonthDayDetail', {
    ...
    refresh: function() {
        ...
        evts = this.store.queryBy(function(rec) {
            var thisDt = Ext.Date.clearTime(this.date, true).getTime(),
                recStart = Ext.Date.clearTime(rec.data[Form.model.reminder.EventMappings.getFields()[0].StartDate.name], true).getTime(),
                startsOnDate = (thisDt == recStart),
                spansDate = false;

            if (!startsOnDate) {
                var recEnd = Ext.Date.clearTime(rec.data[Form.model.reminder.EventMappings.getFields()[0].EndDate.name], true).getTime();
        ...
        evts.each(function(evt) {
            ...
            item._renderAsAllDay = item[M.IsAllDay.name] || Form.view.reminder.util.Date.diffDays(item[M.StartDate.name], item[M.EndDate.name]) > 0;
            item.spanLeft = Form.view.reminder.util.Date.diffDays(item[M.StartDate.name], this.date) > 0;
            item.spanRight = Form.view.reminder.util.Date.diffDays(this.date, item[M.EndDate.name]) > 0;
