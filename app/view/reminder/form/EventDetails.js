 ...
 */
Ext.define('Form.view.reminder.form.EventDetails', {
    ...
    initComponent: function() {
        ...
        this.titleField = new Ext.form.Text({
            fieldLabel: 'Title',
            name: Form.model.reminder.EventMappings.getFields()[0].Title.name,
            emptyText: 'Event Title',
            allowBlank: false,
            anchor: '90%'
        });
        this.dateRangeField = new Form.view.reminder.form.field.DateRange({
            fieldLabel: 'When',
            singleLine: false,
            anchor: '90%'
        });
        this.reminderField = new Form.view.reminder.form.field.ReminderCombo({
            name: 'Reminder',
            anchor: '70%'
        });
        this.notesField = new Ext.form.TextArea({
            fieldLabel: 'Notes',
            name: Form.model.reminder.EventMappings.getFields()[0].Notes.name,
            grow: true,
            growMax: 150,
            anchor: '100%'
        });
        this.locationField = new Ext.form.Text({
            fieldLabel: 'Location',
            name: Form.model.reminder.EventMappings.getFields()[0].Location.name,
            anchor: '100%'
        });
        this.urlField = new Ext.form.Text({
            fieldLabel: 'Web Link',
            name: Form.model.reminder.EventMappings.getFields()[0].Url.name,
            anchor: '100%'
        });
        this.queryField = new Ext.form.TextArea({
            fieldLabel: 'Query',
            height: 300,
            name: Form.model.reminder.EventMappings.getFields()[0].TheQuery.name,
            anchor: '100%'
        });

        var leftFields = [this.titleField, this.dateRangeField, this.reminderField],
        rightFields = [this.notesField, this.locationField, this.urlField, this.queryField];

        if (this.calendarStore) {
            this.calendarField = new Form.view.reminder.form.field.CalendarCombo({
                store: this.calendarStore,
                anchor: '70%',
                allowBlank: false,
                name: Form.model.reminder.EventMappings.getFields()[0].CalendarId.name
            });
    ...
    loadRecord: function(rec){
        ...
        if(this.calendarStore){
            this.form.setValues({
                'calendar': rec.data[Form.model.reminder.EventMappings.getFields()[0].CalendarId.name]
            });
        }
    ...
    onSave: function(){
        ...
        if(this.activeRecord.phantom) {
        	this.fireEvent('eventadd', this, this.activeRecord);
        } else {
        	
        	Ext.re.data.eventUpdate(this.activeRecord.data, function(res) {
        		console.log(res);
        		if(res.data == "success") {
        			
        		} else {
        			Ext.Msg.alert('Error', 'Error in updating the record...');
        		}
        	});
