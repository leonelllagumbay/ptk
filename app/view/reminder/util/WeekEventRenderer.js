 ...
 */
Ext.define('Form.view.reminder.util.WeekEventRenderer', {
        ...
        render: function(o) {
            ...
            for (; w < weekCount; w++) {
                if (!grid[w] || grid[w].length == 0) {
                    ...
                    dt = Form.view.reminder.util.Date.add(dt, {days: 7});
                } else {
                    var row,
                        d = 0,
                        wk = grid[w],
                        startOfWeek = Ext.Date.clone(dt),
                        endOfWeek = Form.view.reminder.util.Date.add(startOfWeek, {days: dayCount, millis: -1});
                        ...
                        if (wk[d]) {
                            ...
                            for (; ev < ct; ev++) {
                                ...
                                if (!evt.isSpan || evt.isSpanStart) {
                                    ...
                                    item._weekIndex = w;
                                    item._renderAsAllDay = item[Form.model.reminder.EventMappings.getFields()[0].IsAllDay.name] || evt.isSpanStart;
                                    item.spanLeft = item[Form.model.reminder.EventMappings.getFields()[0].StartDate.name].getTime() < startOfWeek.getTime();
                                    item.spanRight = item[Form.model.reminder.EventMappings.getFields()[0].EndDate.name].getTime() > endOfWeek.getTime();
                                    ...
                                    var diff = Form.view.reminder.util.Date.diffDays(dt, item[Form.model.reminder.EventMappings.getFields()[0].EndDate.name]) + 1,
