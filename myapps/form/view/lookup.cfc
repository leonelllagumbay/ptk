...
<cftry>
	...
	<cfset tmpresult = StructNew() >
	<cfset tmpresult['linkcode']  = 'DEPARTMENTHEAD' >
	<cfset tmpresult['linkname']  = 'Department Head' >
	...
	<cfset resultArr[7] = tmpresult    >
	
	<cfset tmpresult = StructNew() >
	<cfset tmpresult['linkcode']  = 'TOBESPECIFIED' >
	<cfset tmpresult['linkname']  = 'To be specified' >
	<cfset resultArr[8] = tmpresult    >
...
<cfquery name="qryCount" datasource="#session.company_dsn#">
 SELECT PERSONNELIDNO
   FROM CMFPA
    <cfif query NEQ "" >
	 	WHERE LASTNAME LIKE '%#query#%' OR FIRSTNAME LIKE '%#query#%'
